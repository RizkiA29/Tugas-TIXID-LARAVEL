    ...
    <div class="card-body">
        ...
        <table class="w-100">
            <tr>
                ...
                <td><b>{{ implode(',', json_decode($ticket['row_of_seat'])) }}</b></td>
            </tr>
            <tr>
                <td>Harga Tiket</td>
                <td><b>Rp. {{ number_format($ticket['schedule']['price'])}}<span class="text-secondary" > X{{ $ticket['quantity'] }}</span></b></td>
            </tr>
            <tr>
                <td>Biaya Layanan</td>
        ...
        <div class="d-flex justify-content-end mb-3">
            ...
            @endphp
         <b>Total Bayar : Rp. {{ number_format($price , 0, ',', '.') }}</b>
        </div>
        <form action="{{ route('tickets.payment.status', $ticket->id) }}" method="POST">
            @csrf
            @method('PATCH')
           <button class="btn btn-lg btn-block btn-primary">Sudah Bayar</button>
        </form>
    </div>
</div>
@endsection
