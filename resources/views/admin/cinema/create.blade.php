...
<form method="POST" action="{{route('admin.cinemas.store')}}">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="name" name="name" @error('name') is-invalid @enderror>
        @error('name')
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="location" name="location" rows="5" @error('location') is-invalid @enderror></textarea>
        @error('location')
        <small class="text-danger">{{ $message }}</small>
        @enderror
    </div>
